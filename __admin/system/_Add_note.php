                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card mb-3">
                            ...
                            <div class="card-body">
                                <form submit-ajax="nhq" action="<?=BASE_URL('api/admin/thongbao/add');?>" method="post" api_token="<?=$NHQ->getUsers('token');?>">
                                    <div class="row">
                                        <div class="col-md-12">
                                ...
                                <div class="table-responsive scrollbar">
                                    <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
                                        <table class="table table-bordered table-striped fs--1 mb-3 dataTables-nhq dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                                            ...
                                            <tbody>
                                                <?php $getlistcard = $NHQ->get_list("SELECT * FROM `note_thongbao` WHERE `nguoidang` = '$my_user' ORDER BY `id` DESC "); if($getlistcard){ foreach($getlistcard as $rowucard){?>
                                                <tr class="odd">
                                                    <td><?=$rowucard['id']?></td>
                                                    <td><?=$rowucard['nguoidang']?></td>
                                                    <td><?=$rowucard['noidung']?></td>
                                                    <td><?=$rowucard['date']?></td>
                                                    <td><button type="button" onclick="removenote('<?=$rowucard['id']?>')" class="btn btn-danger btn-sm">Xóa thông báo</button></td>
                                                </tr>
                                                <?php }}else{?>
                                                <tr class="odd">
...
function removenote(idnote){
    Swal.fire({
        title: "Xác nhận!",
        text: `Bạn có muốn xóa thông báo id: ${idnote} ?`,
        icon: "warning",
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: "<?=BASE_URL('api/admin/thongbao/remove');?>",
                headers: {
