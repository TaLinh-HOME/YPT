                                        ...
                                        <table class="table table-bordered table-striped fs--1 mb-3 dataTables-nhq dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                                            <thead class="bg-200 text-900">
                                                <tr role="row" history>
                                                    ...
                                                    <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Link buff: activate to sort column ascending" style="width: 148.969px;">Link buff</th>
                                            ...
                                            <tbody>
                                                <?php $getlistcard = $NHQ->get_list("SELECT * FROM `history_buft` ORDER BY `id` DESC "); if($getlistcard){ foreach($getlistcard as $rowucard){?>
                                                <tr class="odd">
                                                    <td><?=$rowucard['id']?></td>
                                                    ...
                                                    <td><b><?=$rowucard['link_buft']?></b></td>
                                                    <td><?=$rowucard['server_buft']?></td>
                                                    ...
                                                    <td><b>
                                                        ...
                                                        <button type="button" class="btn btn-danger btn-sm" onclick="if (!window.__cfRLUnblockHandlers) return false; reportOrder('<?=BASE_URL('api/admin/order/ok');?>','<?=$rowucard['ma_gd']?>','<?=$rowucard['id']?>');">Duyệt đơn</button>
                                                        <?php }else{?>
                                                        <button type="button" class="btn btn-danger btn-sm" onclick="if (!window.__cfRLUnblockHandlers) return false; cancelOrder('<?=BASE_URL('api/admin/order/cancel');?>','<?=$rowucard['ma_gd']?>','<?=$rowucard['id']?>');">Hủy đơn</button>
                                                        <?php }?>
                                                    </b></td>
                                                </tr>
                                                <?php }}else{?>
