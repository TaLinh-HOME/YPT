            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card mb-3">
                            <div class="card-body">
                                <h6 class="card-title mb-3">Danh sách thành viên - <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#exampleModal">Cộng tiền</button></h6>
                                        ...
                                        <table class="table table-bordered table-striped fs--1 mb-3 dataTables-nhq dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                                            <thead class="bg-200 text-900">
                                                <tr role="row" history>
                                                    ...
                                                    <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Mã nạp tiền: activate to sort column ascending" style="width: 162.031px;">Mã nạp tiền</th>
                                                    ...
                                                    <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Tổng trừ: activate to sort column ascending" style="width: 96.0781px;">Tổng trừ</th>
                                                    <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Thao tác: activate to sort column ascending" style="width: 58.047px;">Thao tác</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <?php $getlistcard = $NHQ->get_list("SELECT * FROM `users` ORDER BY `id` DESC "); if($getlistcard){ foreach($getlistcard as $rowucard){?>
                                                <tr class="odd">
                                                    <td><?=$rowucard['id']?></td>
                                                    <td><?=$rowucard['username']?></td>
                                                    <td><?=$rowucard['cuphap']?></td>
                                                    ...
                                                    <!--</form>-->
                                                    <td>
                                                        <?php if($rowucard['banned'] == 0){?>
                                                        <button type="button" onclick="blockuser('<?=$rowucard['username']?>')" class="btn btn-danger btn-sm">Block</button>
                                                        <?php }else{?>
                                                        <button type="button" onclick="unlockuser('<?=$rowucard['username']?>')" class="btn btn-danger btn-sm">Unlock</button>
                                                        <?php }?>
                                                    </td>
                                                </tr>
                                                <?php }}else{?>
...
function blockuser(keyuser){
    Swal.fire({
        title: "Xác nhận!",
        text: `Bạn có muốn block: ${keyuser} ?`,
        icon: "warning",
...
function unlockuser(keyuser){
    Swal.fire({
        title: "Xác nhận!",
        text: `Bạn có muốn unlock: ${keyuser} ?`,
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
...
$.ajaxSetup({
    ...
    $('#btn-add-corfirm').html('<i class="fas fa-spinner fa-spin"></i> Đang xử lý...').prop('disabled', true);
        $.ajax({
            url: "<?=BASE_URL('api/user/money/add');?>",
            method: "POST",
            headers: {
                    "Api-Token": profile.api_token,
            },
            ...
            success: function(response) {
                ...
                if (response.status === false) {
                    $('#btn-add-corfirm').html('Cộng tiền').prop('disabled', false);
                }
            }
        });
    });
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Cộng tiền cho user</h5>
      ...
      <div class="modal-body">
       <form>
          ...
          <div class="form-group">
            <label for="recipient-money" class="col-form-label">Nhập số tiền:</label>
            <input type="number" class="form-control" id="recipient-money">
          </div>
        </form>
      </div>
