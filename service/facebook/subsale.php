...
// $selectservice = $NHQ->get_row("SELECT * FROM `rate_service` WHERE `code_service` = 'sub_sale' AND `status_service` = '1' "); 
$getrate = $NHQ->get_list("SELECT * FROM `server_service` WHERE `code_service` = 'sub_sale' AND `status_service` = '1' ");
}?>
                    ...
                    <div class="col-md-12">
                        <div class="card mb-3">
                            <div class="card-header">
                                <div class="row">
                                    <div class="col-6"><a href="<?=BASE_URL('service/sub-sale/buy');?>" class="btn btn-primary btn-block">Thêm đơn</a>
                                    </div>
                                    <div class="col-6"><a href="<?=BASE_URL('service/sub-sale/list');?>" class="btn btn-outline-primary btn-block">Danh sách đơn</a>
                                    </div>
                                </div>
                            </div>
                            <div class="card-body">
                                <form submit-ajax="nhq" action="<?=BASE_URL('api/service/sub-sale');?>" method="post" api_token="<?=$NHQ->getUsers('token');?>">
                                        ...
                                        <div class="col-md-9">
                                            ...
                                            <div class="mb-2">
                                                <div class="form-check">
                                                    ...
                                                    <label class="form-check-label" for="<?=$showrate['server_service']?>"><?=$showrate['server_service']?> <small>(<?=$showrate['title_service']?>)</small>&nbsp;<span class="badge border border-success text-success"><?=$showrate['rate_service']?> coin / 1 sub</span>&nbsp;<b class="text-warning">(Hoạt động)</b></label>
                                                </div>
                                            </div>
                                            <?php }?>
                                        </div>
                                    </div>
                                    <div class="alert bg-danger alert-dismissible fade show fs--1" role="alert">
- Mua bằng ID Facebook đã mở chế độ công khai, có nút theo dõi, buff được tài khoản
dưới 18+, và có thể buff cho pro5. <br>
- Sv1 tăng từ 3k -&gt; 15k/ 1 ngày tốc độ ổn định, nhanh hơn với đơn to (tùy vào sub
gốc), có bảo hành tự động trong 1 tháng. <br>
- Sv2 tăng từ 300 -&gt; 5k/ 1 ngày tốc độ chậm, nhanh hơn với đơn to (tùy vào sub gốc),
có bảo hành tự động trong 1 tháng (loại này hiện tại khá đông đơn nên có thể sẽ bị chậm).
<br>
- Sv3 tăng từ 50k -&gt; 100k/ 1 ngày tốc độ cực nhanh, đơn càng to càng nhanh (tùy vào
sub gốc), có bảo hành tự động trong 1 tháng. <br>
- Sv4 sub khuyến mại tốc độ chậm (mua 1k/ 1 lần, chạy xong mới mua tiếp), không bảo
hành (mua sv này rồi thì không mua sv6). <br>
- Sv5 sub khuyến mại tốc độ chậm, mỗi ngày chỉ nhận nhận 100k sub (tự động reset sau
12h đêm), loại này nhiều người săn nên mua nhanh mới có, không bảo hành. <br>
- Sv6 sub khuyến mại tốc độ chậm, mỗi ngày chỉ nhận nhận 10k sub (tự động reset sau
12h đêm),(mua 1k/ 1 lần, chạy xong mới mua tiếp), không bảo
hành (mua sv này rồi thì không mua sv4). <br>
- Sau khi mua sub sẽ lên sau 10p -&gt; 1h. <br>
- Không mua nhiều bên hoặc nhiều Sv cho 1 ID, tụt hay thiếu không hỗ trợ. <br>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-12 text-center">
                                            <div class="alert alert-dismissible alert-success">
                                                <center><h3 class="font-bold">Tổng thanh toán: <span class="bold green"><span id="total_payment" class="text-danger"><?=$showrate['tong_tien']?> </span> coin</span></h3></center>
                                            </div>
                                        </div>
                                    </div>
