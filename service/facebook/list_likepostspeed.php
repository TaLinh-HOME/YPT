                    ...
                    <div class="col-md-12">
                        <div class="card mb-3">
                            <div class="card-header">
                                <div class="row">
                                    <div class="col-6"><a href="<?=BASE_URL('service/like-post-speed/buy');?>" class="btn btn-outline-primary btn-block">Thêm đơn</a>
                                    </div>
                                    <div class="col-6"><a href="<?=BASE_URL('service/like-post-speed/list');?>" class="btn btn-primary btn-block">Danh sách đơn</a>
                                    </div>
                                </div>
                            </div>
                            <div class="card-body">
                                ...
                                <div class="table-responsive scrollbar">
                                    ...
                                    <table class="table table-striped table-bordered mt-4 dataTables-nhq dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                                        ...
                                        <tbody>
                                            <?php $getlistbuff = $NHQ->get_list("SELECT * FROM `history_buft` WHERE `type` = 'like_post_speed' AND `username` = '$my_user' ORDER BY `id` DESC "); if($getlistbuff){ foreach($getlistbuff as $rowhbuff){?>
                                            <tr class="odd">
                                                <td><?=$rowhbuff['id']?></td>
                                                <td><?=$rowhbuff['date']?></td>
                                                <td><?=$rowhbuff['ma_gd']?></td>
                                                <td><?=$rowhbuff['link_buft']?></td>
                                                <td><span class="badge badge-primary"><?=$rowhbuff['server_buff']?></span>
                                                </td>
                                                <td><b class="text-danger"><?=$rowhbuff['soluong']?></b><sup>like</sup>
                                                </td>
                                                <td><b class="text-danger"><?=$rowhbuff['da_buft']?></b><sup>like</sup>
                                                </td>
                                                <td><b class="text-danger"><?=$rowhbuff['tong_tien']?></b><sup>coin</sup>
                                                </td>
                                                <td><b><?=statusorder($rowhbuff['status'])?></b></td>
                                                <td>
                                                    <button type="button" class="btn btn-danger btn-sm" onclick="if (!window.__cfRLUnblockHandlers) return false; cancelOrder('<?=BASE_URL('api/service/like-post-speed/cancel');?>','<?=$rowhbuff['ma_gd']?>','<?=$rowhbuff['id']?>');">Hủy đơn</button>
                                                </td>
                                            </tr>
                                            <?php }}?>
                                        </tbody>
