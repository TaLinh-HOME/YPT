...
if ( $_SERVER['REQUEST_METHOD'] == 'GET' && realpath(__FILE__) == realpath( $_SERVER['SCRIPT_FILENAME'] ) ) {
    ...
    ****/
    ...
    if(empty($server_order)){
        ...
        $selectservice = $NHQ->get_row("SELECT * FROM `server_service` WHERE `code_service` = 'eyevideo' AND `status_service` = '1' AND `server_service` = '$server_order'");
    ...
    $code_order = 'ORDER_'.random('QWERTYUIOPASDFGHJKLZXCVBNM1234567890',10).rand(0,99999);
        ...
        if($create){
            ...
            apitele('tangsub.vn bạn có đơn hàng chưa xử lý buff mắt video của '.$checkusite['username'].' 
           link buff: '.$link_post.'
           số lượng: '.$amount.'
           lời nhắn riêng: '.$note.'
           tổng tiền: '.number_format($tongtien).'đ
            ...
            if($update){
                ...
                'note'          => $checkusite['username'].' sử dụng buff eyevideo với giá tiền '.$tongtien.' vào lúc '.gettime().'',
                'ip'            => getip(),
                'date'          => gettime()
                ]);
