...
if ( $_SERVER['REQUEST_METHOD'] == 'GET' && realpath(__FILE__) == realpath( $_SERVER['SCRIPT_FILENAME'] ) ) {
    ...
    ****/
    ...
    if(!$NHQ->get_row("SELECT * FROM `history_buft` WHERE `type` = 'comment_sale' AND `ma_gd` = '$code_order' AND `username` = '".$checkusite['username']."' ")){
        $return['status'] = false;
        $return['error'] = true;
    ...
    $selectservice = $NHQ->get_row("SELECT * FROM `history_buft` WHERE `type` = 'comment_sale' AND `ma_gd` = '$code_order' ");
